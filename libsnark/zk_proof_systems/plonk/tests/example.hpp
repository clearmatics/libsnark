...
public:
    ...
    /// hard-coded gates matrix for the example circuit P(x) = x**3 +
    /// x + 5 = 3 Each column is a q-vector
    ...
    /// wire permutation (TODO: add function
    /// plonk_compute_permutation())
    ...
    /// omega_base**(2**32) = 1. The bls12-381 prime q is such that
    /// any power of 2 divides (q-1). In particular 2**32|(q-1) and so
    /// the 2**32-th root of unity exists.
    Field omega_base;

    /// Constants k1,k2 to generate domains on which to evaluate the
